...
// NOTE: WorkUnits should not be put out globally or they might cause failure in tests

func BenchmarkWorkUnit_Shuffle(b *testing.B) {
	var valToShuf = []byte("goshufflemlib@vaibhavkaushal.com")
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		workUnit.Input = valToShuf
		_ = workUnit.Shuffle()
	}
}

func TestWorkUnit_Shuffle(t *testing.T) {
	var valToShuf = []byte("goshufflemlib@vaibhavkaushal.com")
	var workUnit = WorkUnit{
		Input:        valToShuf,
		BitSetterMap: make(map[uint16]bool),
	...
	if err != nil {
		t.Errorf("E#O4OBL - Error: %v", err)
	}

	expectedResult := []byte{182, 246, 198, 116, 54, 134, 22, 206, 174, 134, 214, 110, 134, 22, 70, 150, 134, 110, 2, 70, 150, 54, 182, 166, 54, 102, 102, 174, 22, 206, 246, 230}

	if len(workUnit.Output) != len(expectedResult) {
	...
	for i := 0; i < len(workUnit.Output); i++ {
		if workUnit.Output[i] != expectedResult[i] {
			t.Errorf("E#O4PHE - Expected %v, got %v", expectedResult[i], workUnit.Output[i])
		}
	}
}
